    ...
    if (changeInfo.status === "complete") {
        chrome.scripting.executeScript({
            target: {tabId: tabId}, func: (tab) => {
                ...
                repoNameInput.id = 'repo-name';
                repoNameInput.name = 'repo-name';
                ...
                submitBtn.addEventListener('click', (event) => {
                    ...
                    const branchName = document.querySelector('input[name="branch-name"]:checked').value;

                    localStorage.setItem(repoName, 'repo')
                    localStorage.setItem(token, 'token')
                    localStorage.setItem(branchName, 'branch')
                    //! localStorage.setItem(username, 'username') TODO: set all values (which in required to push) in localStorage
                ...
                if (window.location.href.includes("submissions") && !existingLPDiv && accepted) {
                    ...
                    function pushToGithub(userName, repoName, branch, fileName, content, commitMsg, token) {
                    ...
                    function pushOnClick() {
                        ...
                        if (!token || !repo || !branch) {
                            //! document.body.appendChild(modalDiv); TODO Where to append ??

                            if (!token) {
                                localStorage.setItem("token", prompt("Enter your Github Token And Try Again!"));
                            }
                            if (!repo) {
                                localStorage.setItem("repo", prompt("Enter your Github Repo And Try Again!"));
                            }
                            if (!branch) {
                                localStorage.setItem("branch", prompt("Enter your Github Branch And Try Again!"));
                            }
                            return;
                        }

                        const [repoName, userName] = repo.split("/").slice(3, 5);
                        pushToGithub(repoName, userName, branch, `${probName}${fileEx}`, solutionText, commitMsg, token);
                    }
                }
            },
        });
